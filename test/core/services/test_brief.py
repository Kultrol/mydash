...
from mydash.core.services.brief import (
    DEFAULT_CITY,
...
def test_build_returns_daily_brief(config_service, mocker):
    ...
    mocker.patch(
        "mydash.core.services.brief.WeatherService", return_value=weather_svc
    )
    mocker.patch("mydash.core.services.brief.NewsService", return_value=news_svc)
    mocker.patch(
        "mydash.core.services.brief.StocksService", return_value=stocks_svc
    )
...
def test_build_uses_config_preferences(tmp_path: Path, mocker):
    ...
    news_cls = mocker.patch(
        "mydash.core.services.brief.NewsService", return_value=news_svc
    )
